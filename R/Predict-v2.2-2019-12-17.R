...
calculate <-
  function(## ----input, echo=FALSE---------------------------------------------------
           age.start  = 66,
           screen     = 1,
           # Clinically detected = 0, screen detected = 1
           size       = 2,
           # Tumour size mm
           grade      = 1,
           # Tumour grade
           ...
           # ER+ = 1, ER- = 0
           her2       = 1,
           # HER2+ = 1, HER2- = 0, missing = 9
           ki67       = 1,
           # KI67+ = 1, KI67- = 0, missing = 9
           generation = 3,
           # Chemo generation 0, 2 or 3 only
           horm       = 1,
           # Hormone therapy Yes = 1, no = 0
           traz       = 1,
           # Trastuzumab therapy Yes = 1, no = 0
           bis        = 1,
           # Bisphosphonate therapy Yes = 1, no = 0
           radio      = 0)
           # Radiotherapy Yes = 1, no = 0
  {
    r.enabled  <-
      0     # Radiotherapy enabled = 1, disabled = 0

    ##----------------------------------------------------------------
    ##[WINTON FIX] Fix inputs
    screen    <-
      ifelse(screen == 2, 0.204, screen)
    grade     <-
      ifelse(grade == 9, 2.13, grade)
    ...
    mi <-
      0.0698252 * ((age.start / 10) ^ 2 - 34.23391957) + r.base.oth
    ...
    h     <-
      ifelse(horm == 1 & er == 1,-0.3857, 0)
    t     <-
      ifelse(her2 == 1 & traz == 1,-.3567, 0)
    b     <-
    ...
    cb <-
      c + b # in a woman with ER positive disease
    tb <- t + b
    hct <- h + c + t
    hcb <- h + c + b
    htb <- h + t + b
    ...
    cols <-
      length(rx.oth) # Number of RX categories
    ...
    if (er == 1) {
      ...
      base.m.cum.br <-
        exp(-1.156036 + 0.4707332 / time ^ 2 - 3.51355 / time)
    }
    ...
    m.br.rx <-
      sapply(rx, function(rx, x.vector = base.m.br) {
    ...
    prop.br.rx      <-
      m.br.rx / (m.br.rx + m.oth.rx)
    pred.m.br.rx    <- prop.br.rx * m.all.rx
    pred.cum.br.rx  <-
      apply(pred.m.br.rx, 2, cumsum)
    pred.m.oth.rx   <- m.all.rx - pred.m.br.rx
    pred.cum.oth.rx <-
      apply(pred.m.oth.rx, 2, cumsum)
    pred.cum.all.rx <-
      pred.cum.br.rx + pred.cum.oth.rx
    ...
    base.rel.rx <-
      sapply(rx, function(rx, x.vector = base.rel) {
        output <-  x.vector * exp(pi + rx)
        return(output)
      })
    ...
    prop.rel.rx <-
      rel.rx / (rel.rx + m.oth.rx)

    # Calulate the predicted relapse and other mortality
    pred.m.rel.rx       <-
      prop.rel.rx * m.all.rx
    pred.cum.rel.rx     <-
      apply(pred.m.rel.rx, 2, cumsum)
    pred.m.oth.rx       <-
      m.all.rx - pred.m.rel.rx
    pred.cum.oth.rx     <-
      apply(pred.m.oth.rx, 2, cumsum)
    pred.cum.all.rel.rx <-
      pred.cum.rel.rx + pred.cum.oth.rx
    ...
    ## ------------------------------------------------------------------------
    delay <-
      5   # Set delay to 0 to check predictions with main model
    rows  <- 15 - delay
    time  <- c(1:rows)
    ...
    # Generate the annual breast cancer specific mortality rate
    m.br.rx.10 <-
      base.m.br[(1 + delay):15] * exp(rx10)

    # Calculate the cumulative breast cancer mortality rate
    m.cum.br.rx.10 <-
      apply(m.br.rx.10, 2, cumsum)
    ...
    m.cum.all.rx.10 <-
      1 - s.cum.oth.10 * s.cum.br.rx.10
    s.cum.all.rx.10 <-
      100 - 100 * m.cum.all.rx.10
    ...
    for (j in 1:2) {
      for (i in 2:rows) {
        m.all.rx.10[i, j] <-
          m.cum.all.rx.10[i, j] - m.cum.all.rx.10[i - 1, j]
      }
    }

    # Proportion of all cause mortality that is breast cancer
    prop.br.rx.10      <-
      m.br.rx.10 / (m.br.rx.10 + m.oth.10)
    pred.m.br.rx.10    <-
      prop.br.rx.10 * m.all.rx.10
    pred.cum.br.rx.10  <-
      apply(pred.m.br.rx.10, 2, cumsum)
    pred.m.oth.rx.10   <-
      m.all.rx.10 - pred.m.br.rx.10
    pred.cum.oth.rx.10 <-
      apply(pred.m.oth.rx.10, 2, cumsum)
    pred.cum.all.rx.10 <-
...
fn.benefits2.1 <-
  function(## ----input, echo=FALSE---------------------------------------------------
           ...
           radio      = 0)
           # Radiotherapy Yes = 1, no = 0
  { calculate.all(age.start,screen,size,grade,nodes,er,her2,ki67,generation,horm,traz,bis,radio);
    return(benefits2.1)
    }
